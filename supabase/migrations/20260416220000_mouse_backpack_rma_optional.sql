-- Muis- en rugzak-RMA: geen verplicht serienummer of defect_description (reden).
...
BEGIN
  IF NEW.line_type = 'rma_defect' THEN
    ...
    ELSIF NEW.rma_category = 'voedingskabel' THEN
      ...
      END IF;
    ELSIF NEW.rma_category = 'headset' THEN
      IF NEW.defect_description IS NULL OR btrim(NEW.defect_description) = '' THEN
        RAISE EXCEPTION 'Headset-vervanging vereist defect_description (reden)';
      END IF;
    END IF;
  END IF;
  RETURN NEW;
